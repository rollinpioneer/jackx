...
def main(web_mode=False):
    ...
    if not article_text or not reference_summary:
        print("错误: 无法读取数据。请先运行mt5Summary.py生成数据。")
        return
    ...
    # 确保所有摘要的每个句子前没有空格
    summaries = {
        'mT5': clean_sentence_spaces(mt5_summary),
    ...
    # 保存摘要结果时确保每个句子前没有空格
    with open("c:\\Users\\jackx\\Desktop\\transformers\\othersmt5\\mt5TextRank_results.txt", "w", encoding="utf-8") as f:
    ...
    if not web_mode:
        print("\n摘要结果已保存到mt5TextRank_results.txt")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='生成TextRank摘要')
