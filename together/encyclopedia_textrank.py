...
def read_summarization_data():
    ...
    try:
        file_path = os.path.join(os.path.dirname(__file__), "encyclopedia_mt5_summary.txt")
...
def main(web_mode=False):
    ...
    if not article_text:
        print("错误: 无法读取数据。请先运行encyclopedia_mt5.py生成数据。")
        return
    ...
    # 为百科知识文本确定合适的摘要句子数 - 百科文本通常需要更多的内容
    sentences = re.split('([。！？])', article_text)
    sentences = [''.join(i) for i in zip(sentences[0::2], sentences[1::2] + [''])]
    sentences = [sent.strip() for sent in sentences if len(sent.strip()) > 0]
    
    # 根据百科文本长度确定摘要句子数
    sentences_count = max(3, min(7, int(len(sentences) * 0.25)))  # 至少3句，最多7句，或者原文25%的句子数
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        ...
        f.write(f"参考摘要: {clean_sentence_spaces(reference_summary)}\n\n")
        for method, summary in summaries.items():
            f.write(f"{method}摘要: {summary}\n\n")
    
    if not web_mode:
        print("\n摘要结果已保存到encyclopedia_textrank_results.txt")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='生成百科知识TextRank摘要')
